...
class Model(nn.Module):
    ...
    def __init__(self, conv1: nn.Module, pool1: nn.Module, conv2: nn.Module, pool2: nn.Module, 
        ...
        self.conv1, self.pool1 = conv1, pool1
        self.conv2, self.pool2 = conv2, pool2
        self.conv3, self.pool3 = conv3, pool3
        self.conv4, self.pool4 = conv4, pool4
    ...
    def __repr__(self):
        layers = []
        for i in range(1, 6):  
            layers.append(f"(conv{i}): {getattr(self, f'conv{i}')}")
            layers.append(f"(nl): {self.nl}")
            layers.append(f"(pool{i}): {getattr(self, f'pool{i}')}")
        layers.append(f"(last): {self.last}")
        return "Model(\n  " + "\n  ".join(layers) + "\n)"
...
class Expansion5L:
    ...
    def build(self):
        ...
        if self.filters_1 is None: # Pre-fixed filters for layer 1
