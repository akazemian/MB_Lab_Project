...
class WaveletConvolution(nn.Module):
    """
    A convolutional layer that uses pre-defined filters.
    ...
    """
    def __init__(self, filter_type: str, filter_size: int = 15, device: Optional[str] = None) -> None:
        super().__init__()
    ...
    def __repr__(self):
        return f"WaveletConvolution(3, {self.layer_size},  kernel_size=({self.filter_size},{self.filter_size}), filter_type={self.filter_type}, filter_params={self.filter_params})"
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(in_channels):
            ...
            convolved_tensor.append(channel_convolved.to(self.device))
